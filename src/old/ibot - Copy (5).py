...
from interpret import free_interprete
...
base_completer = WordCompleter([':create', ':delete', ':make', ':clone', ':define', ':analyse', ':list', ':freeform'])
...
def make_context(bot_name, bot_type, bot_dir, dirs):
        bot_dir = "../bots/"
        new_dir = os.path.join(bot_dir, bot_name)
        os.mkdir(new_dir)
        for d in dirs:
